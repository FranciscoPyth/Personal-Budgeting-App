...
const { Gastos, MediosDePago, Divisas, TiposTransaccion, Categorias, Relaciones } = require("../models");
...
router.get("/api/gastos", async (req, res) => {
  try {
    ...
    let items = await Gastos.findAndCountAll({
      include: [MediosDePago, Divisas, TiposTransaccion, Categorias, Relaciones],
      order: [["fecha", "ASC"]],
      where,
    });
